...
add_action('woocommerce_order_status_processing', function($order_id) {
	...
	$webhook_url = MasterHotelConfig::get_config('order_reserve_webhook_url');
	...
	if (class_exists('MasterHotelCurlHelper')) {
		$result = MasterHotelCurlHelper::post_json($webhook_url, $order_data);
		
		// Save the webhook result to the order
		$order->update_meta_data('master_spa_order', json_encode($result));
		$order->save();
		
		// Check for errors in the response
		if (is_array($result) && (
			!empty($result['error']) || 
			(isset($result['http_code']) && $result['http_code'] >= 400) ||
			(isset($result['response']) && is_string($result['response']) && 
			 (stripos($result['response'], 'error') !== false || stripos($result['response'], 'fail') !== false))
		)) {
			// Display error message and clear cart
			wc_add_notice('Camera ta nu mai este valabila', 'error');
			WC()->cart->empty_cart();
		} else {
			// Save success status
			$order->update_meta_data('_webhook_status', 'success');
			$order->save();
		}
	}
});
